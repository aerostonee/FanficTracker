...
function initializeEventListeners() {
    ...
    document.getElementById('form-link').addEventListener('input', (e) => {
        const normalized = normalizeAO3Link(e.target.value);
        checkDuplicateLink(normalized);
    });
...
async function checkDuplicateLink(link) {
    ...
    const normalizedLink = normalizeAO3Link(link);
    ...
    const submitBtn = document.querySelector('#fic-form button[type="submit"]');

    if (data.length > 0 && data[0].id !== editingFicId) {
...
function normalizeAO3Link(link) {
    if (!link) return '';

    const match = link.match(/\/works\/(\d+)/);
